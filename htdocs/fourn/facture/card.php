		...
		if (GETPOST('type') == FactureFournisseur::TYPE_STANDARD || GETPOST('type') == FactureFournisseur::TYPE_DEPOSIT && GETPOST('fac_rec') <= 0) {
			...
			if (!$error) {
				...
				if (!$error && GETPOST('origin', 'alpha') && GETPOST('originid')) {
					...
					$originids = preg_split('/,/', $originid);

					if (count($originids) == 1) {
						// classic one
						$object->origin_id = $originids[0];
					}

					$object->linkedObjectsIds[$object->origin] = [];
					$object->linkedObjectsIds['order_supplier'] = [];

					$srcobjects = [];

					foreach($originids as $originidtmp) {
						$objectsrc = new $classname($db);
						$objectsrc->fetch($originidtmp);
						$objectsrc->fetch_thirdparty();

						$srcobjects[] = $objectsrc;

						$object->linkedObjectsIds[$object->origin][] = $originidtmp;
						...
						if ($object->origin == 'reception') {
							...
							if (count($objectsrc->linkedObjectsIds['order_supplier']) > 0) {
								foreach ($objectsrc->linkedObjectsIds['order_supplier'] as $key => $value) {
									$object->linkedObjectsIds['order_supplier'][$value->id] = $value;
								}
							}
						}
					...
					if ($id > 0) {
						...
						foreach($originids as $originidtmp) {
							$result = $srcobject->fetch($originidtmp);
...
if ($action == 'create') {
	...
	if (!empty($origin) && !empty($originid)) {
		...
		foreach($originids as $originidtmp) {
			...
			$cond_reglement_ids[] 	= $objectsrc->cond_reglement_id;
			$mode_reglement_ids[] 	= $objectsrc->mode_reglement_id;
			$fk_accounts[]         = $objectsrc->fk_account;
			$remises_percents[] 	= $objectsrc->remise_percent;
			$remises_absolues[] 	= $objectsrc->remise_absolue;

			$transport_mode_ids[] = (!empty($objectsrc->transport_mode_id) ? $objectsrc->transport_mode_id : (!empty($soc->transport_mode_id) ? $soc->transport_mode_id : 0));
		...
		// Only set the value if all origins have the same information
		$cond_reglement_id 	= (count(array_unique($cond_reglement_ids)) == 1) ? $cond_reglement_ids[0] : ((!empty($soc->cond_reglement_supplier_id) ? $soc->cond_reglement_supplier_id : 0)); // TODO maybe add default value option
		$mode_reglement_id 	= (count(array_unique($mode_reglement_ids)) == 1) ? $mode_reglement_ids[0] : ((!empty($soc->mode_reglement_supplier_id) ? $soc->mode_reglement_supplier_id : 0));
		$fk_account         = (count(array_unique($fk_accounts)) == 1) ? $fk_accounts[0] : ((!empty($soc->fk_account) ? $soc->fk_account : 0));
		$remise_percent 	= (count(array_unique($remises_percents)) == 1) ? $remises_percents[0] : ((!empty($soc->remise_supplier_percent) ? $soc->remise_supplier_percent : 0));
		$remise_absolue 	= (count(array_unique($remises_absolues)) == 1) ? $remises_absolues[0] : ((!empty($soc->remise_absolue) ? $soc->remise_absolue : 0));
		$transport_mode_id 	= (count(array_unique($transport_mode_ids)) == 1) ? $transport_mode_id[0] : ((!empty($soc->transport_mode_id) ? $soc->transport_mode_id : 0));
		$dateinvoice = empty($conf->global->MAIN_AUTOFILL_DATE) ?-1 : '';
	...
	*/
	...
	foreach($srcobjects as $objectsrc) {
		if (is_object($objectsrc)) {
			...
			$objectsrc->fetchObjectLinked($objectsrc->row, $origin, '', 'invoice_supplier');
