...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_jdc.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from JDC model
 */
 ...
 */
class pdf_rms extends ModelePDFFactures
{
	 /**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "RMS";
		$this->description = $langs->trans('PDFRMSDescription');
	...
	protected function _tableau_versements(&$pdf, $object, $posy, $outputlangs, $heightforfooter = 0)
	{
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				if ($obj->array_options['options_internal']) {
					$text .= 'internal';
				}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		if (!$object->array_options['options_internal']) {
			...
			$title = $outputlangs->transnoentities("PdfInvoiceTitleInternal");
			if ($object->type == 1) {
				$title = $outputlangs->transnoentities("InvoiceReplacementInternal");
			}
			if ($object->type == 2) {
				$title = $outputlangs->transnoentities("InvoiceAvoirInternal");
			}
			if ($object->type == 3) {
				$title = $outputlangs->transnoentities("InvoiceDepositInternal");
			}
			if ($object->type == 4) {
				$title = $outputlangs->transnoentities("InvoiceProFormaInternal");
			}
			if ($this->situationinvoice) {
				$title = $outputlangs->transnoentities("PDFInvoiceSituationInternal");
			}
		}
		...
		if ($showaddress) {
			...
			if (empty($conf->global->MAIN_PDF_HIDE_SENDER_NAME)) {
				...
				$pdf->SetFont('', 'B', $default_font_size);
				$pdf->MultiCell($widthrecbox - 2, 4, $outputlangs->convToOutputCharset($this->emetteur->label), 0, $ltrdirection);
				$posy = $pdf->getY();
			}
			...
			$phoneNumber = $outputlangs->convToOutputCharset($this->emetteur->phone);
			$email = $outputlangs->convToOutputCharset($this->emetteur->accountancy_email);
		...
		$pdf->MultiCell(40, 4, 'Notre Référence', 1, 'C', true, 1, $posx, $posy);
		$pdf->MultiCell(40, 4, 'N° Facture', 1, 'C', true, 1, $posx + 40, $posy);
		$pdf->MultiCell(40, 4, 'Date facture', 1, 'C', true, 1, $posx + 80, $posy);
		$pdf->MultiCell(40, 4, 'Date d\'échéance', 1, 'C', true, 1, $posx + 120, $posy);
		$pdf->MultiCell(30, 4, 'Total à payer', 1, 'C', true, 1, $posx + 160, $posy);

		$orderRef = '42';

		$total_ht = ((!empty($conf->multicurrency->enabled) && isset($object->multicurrency_tx) && $object->multicurrency_tx != 1) ? $object->multicurrency_total_ht : $object->total_ht);
		$total_ttc = ((!empty($conf->multicurrency->enabled) && isset($object->multicurrency_tx) && $object->multicurrency_tx != 1) ? $object->multicurrency_total_ttc : $object->total_ttc);
		//$total_ttc = (!empty($conf->multicurrency->enabled) && $object->multicurrency_tx != 1) ? $object->multicurrency_total_ttc : $object->total_ttc;
		...
		$date_lim_reglement = dol_print_date($object->date_lim_reglement, 'day', false, $outputlangs, true);
		...
		foreach ($object->linkedObjects as $objecttype => $objects) {
			if ($objecttype == 'commande') {
				foreach ($objects as $elementobject) {
					...
					$orderRef = $outputlangs->transnoentities($elementobject->ref);
				}
			}
		}
		if (!$orderRef) {
		...
		$pdf->MultiCell(40, 4, $orderRef, 1, 'C', false, 1, $posx, $posy);
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		$pdf->SetXY($posx, $posy);
		$pdf->MultiCell(190, 4, $outputlangs->trans('RMSAnyQuestion'), 0, 'L');

		$posy += 15;
		$pdf->SetXY($posx, $posy);
		$pdf->MultiCell(190, 4, $outputlangs->transnoentities('RMSCGU'), 0, 'L');
