 ...
 */
class mod_propale_jdc extends ModeleNumRefPropales
{
	/**
	...
	{
		global $langs;
		return $langs->trans("JDCPropaleRef");
	}
	...
	{
		return "D22127-001 ou D22127-001-v2";
	}
	...
	public function getExistingPropales($project)
	{
		global $db;

		$sql = strtr(
			"SELECT rowid FROM {propal_table} WHERE fk_projet = {project_id} ORDER BY ref DESC",
			[
				"{propal_table}" => MAIN_DB_PREFIX."propal",
	...
	public function getNextValue($objsoc, $propal)
	{
		...
		$quotation = new Project($db);
		$quotation->fetch($propal->fk_project);
		...
		$customerNumber = 1;
		$maxCustomerNumber = 0;
		foreach($existingPropales as $existingPropal) {

			$ref = $existingPropal->ref;
			$matches = null;
			if (preg_match($regexp, $ref, $matches)) {
		...
		if (!$found) {
			$customerNumber = $maxCustomerNumber+1;
		} else {
			$version = $maxVersion+1;
		}

		$ref = $quotation->ref . '-' . str_pad($customerNumber, 3, "0", STR_PAD_LEFT);
		if ($version > 1) {
			$ref .= '-v' . $version;
		}
